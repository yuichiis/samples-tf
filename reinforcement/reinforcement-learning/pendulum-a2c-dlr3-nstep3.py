...
# === 正規分布関数 ===
def calculate_normal_dist_sampled(mu,log_std):
    """tensorflow-probabilityなしで正規分布からサンプリング"""
    std = tf.exp(log_std)

    # sample = mean + norm*std
    sample = tf.random.normal(
        shape=tf.shape(mu), mean=mu, stddev=std
    )
    return sample
...
def get_action(actor_model, state, action_bound):
    ...
    #action_normalized = calculate_normal_dist_sampled(scaled_mu,log_std)
...
if __name__ == '__main__':
    ...
    actor_model_file = 'pendulum-a2c-dlr3-actor3.weights.h5'
    critic_model_file = 'pendulum-a2c-dlr3-critic3.weights.h5'
