...
class ActorModel(keras.Model):
    def __init__(self, input_shape, action_dim, init_std=1.0):
        super().__init__()
        ...
        std = np.log(np.array(init_std,dtype=np.float32))
        self.logstds = tf.Variable(tf.fill((action_dim,),std), trainable=True, name="logstds")
...
# === 正規分布関数 ===
def calculate_normal_dist_sampled(mu_normalized,log_std):
...
def calculate_normal_dist_stats(mu, log_std, actions):
    """
...
def create_actor_critic_models(input_shape, action_dim, init_std=1.0):
    """
    ...
    actor_model = ActorModel(input_shape, action_dim, init_std)
...
def get_action(actor_model, state, action_bound):
    ...
    action_normalized = calculate_normal_dist_sampled(mu_normalized,log_std)
    
    no_cliped_action = action_normalized[0] * action_bound
    ...
    #    log_std[0].numpy(),
    #    tf.math.exp(log_std)[0].numpy(),
    #    mu_normalized[0,0].numpy(),
...
def update(actor_model, critic_model, actor_optimizer, critic_optimizer, experiences, gamma, value_loss_weight, entropy_weight, standardize, action_bound):
    ...
    with tf.GradientTape() as actor_tape:
        ...
        log_prob, entropy = calculate_normal_dist_stats(mu_scaled, log_std, actions)
...
def train(env, actor_model, critic_model, action_bound):
    ...
    print('init_std =', np.exp(actor_model.logstds.numpy()))
    ...
    for global_step in range(1, total_timesteps + 1):
        ...
        if (global_step % (n_steps * 5) == 0) or global_step == total_timesteps:
            ...
            # ログ表示を更新
            std = np.mean(tf.exp(actor_model.logstds).numpy())
            print(f'St:{global_step//1000}k|Ep:{episode_count}|AvgSt:{avg_episode_steps:.1f}|AvgRwd:{avg_reward:.1f}|'
                  f'ActorL:{last_actor_loss:.3f}|CriticL:{last_critic_loss:.3f}|Entr:{last_entropy:.3f}|Std:{std:.3f}')
...
if __name__ == '__main__':
    ...
    actor_model, critic_model = create_actor_critic_models(obs_shape, action_dim, init_std)
    print("--- Actor Model ---")
    actor_model.summary()
    print("\n--- Critic Model ---")
    critic_model.summary()

    actor_model_file = 'pendulum-a2c-dlr3-actor1.weights.h5'
    critic_model_file = 'pendulum-a2c-dlr3-critic1.weights.h5'
    ...
    gif_path = 'pendulum-a2c-dlr3-nstep1.gif'
