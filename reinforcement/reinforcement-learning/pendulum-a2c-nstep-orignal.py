...
def create_a2c_model(input_shape, action_dim):
    ...
    #
    # log_stdを直接出力する層
    log_std_output = keras.layers.Dense(action_dim, name="log_std_raw")(common_layer)
    ...
    model = keras.Model(inputs=input_layer, outputs=[actor_mu, log_std_output, critic_value])
    return model
...
def update(model, optimizer, experiences, gamma, value_loss_weight, entropy_weight, standardize, action_bound):
    ...
    with tf.GradientTape() as tape:
        ...
        # 1. log_prob (対数確率密度) を数式から直接計算
        # 正規分布のPDF: f(x) = (1 / (sqrt(2*pi*sigma^2))) * exp(-(x-mu)^2 / (2*sigma^2))
        # log f(x) = -log(sigma) - 0.5*log(2*pi) - 0.5*((x-mu)/sigma)^2
        #          = -0.5 * (2*log(sigma) + log(2*pi) + ((x-mu)/sigma)^2)
        log_prob = -0.5 * (tf.square((actions - mu_scaled) / std) + 2 * log_std + np.log(2 * np.pi))
        log_prob = tf.reduce_sum(log_prob, axis=1, keepdims=True) # action_dim > 1 の場合に対応

        # 2. エントロピーを数式から直接計算
        # 正規分布のエントロピー: H = 0.5 * log(2*pi*e*sigma^2)
        #                        = 0.5 + 0.5*log(2*pi) + log(sigma)
        entropy = 0.5 + 0.5 * tf.math.log(2 * np.pi) + log_std
        entropy = tf.reduce_sum(entropy, axis=1, keepdims=True) # action_dim > 1 の場合に対応
        # ★★★ 変更点ここまで ★★★
