...
class A2CAgent:
    ...
    def compute_discounted_rewards(self, rewards, dones):
        """Discounted Rewardを計算（エピソード終了から先頭に向かって）"""
        ...
        for t in reversed(range(len(rewards))):
            if dones[t]:
                running_reward = 0  # エピソード終了時はリセット
    ...
    def train(self, states, actions, rewards, dones):
        ...
        with tf.GradientTape() as tape:
            ...
            # Advantage = Discounted Reward - Value
            advantages = discounted_rewards - values
            
            # Critic Loss (Value関数の誤差)
    ...
    for episode in range(MAX_EPISODES):
        ...
        if len(episode_states) > 0:
            critic_loss, actor_loss = agent.train(
                episode_states, episode_actions, episode_rewards_step, episode_dones
            )
