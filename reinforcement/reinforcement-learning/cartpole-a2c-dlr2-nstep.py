...
# === 学習関数 (修正済み) ===
def update(model, actor_optimizer, critic_optimizer, experiences, gamma, value_loss_weight, entropy_weight, standardize):
    ...
    policy_gradients = tape.gradient(policy_loss, model.trainable_variables)
    value_gradients = tape.gradient(scaled_value_loss, model.trainable_variables)
    actor_optimizer.apply_gradients(zip(policy_gradients, model.trainable_variables))
    critic_optimizer.apply_gradients(zip(value_gradients, model.trainable_variables))
...
def train(env,model):
    ...
    print('A2C Different Learning Rates版Ⅱ')
    ...
    for global_step in range(1, total_timesteps + 1):
        ...
        if len(experiences) >= n_steps:
            p_loss, v_loss, entropy = update(model, actor_optimizer, critic_optimizer, experiences, gamma, value_loss_weight, entropy_weight, standardize)
        ...
        if (global_step % (n_steps*10) == 0) or global_step == total_timesteps:
            ...
            val_reward, val_step = evaluation(model)
...
if __name__ == '__main__':
    ...
    model_file = 'cartpole-a2c-dlr2-nstep.weights.h5'
    ...
    env_render.close()
    imageio.mimsave('cartpole-a2c-dlr2-nstep.gif', frames, fps=30)
    print(f"GIFを'cartpole-a2c-dlr2-nstep.gif'に保存しました。最終報酬: {test_reward:.2f}")
