...
class ActorModel(keras.Model):
    def __init__(self, input_shape, action_dim):
        ...
        self.mu_head = keras.layers.Dense(
            action_dim, name="actor_mu", kernel_initializer=last_init
        )

        # log_stdを出力するヘッド
        # こちらの初期化はゼロに近い値（または少し負の値）にすると、
        # 学習初期のstdが1.0（またはそれ以下）から始まり安定しやすい
        log_std_init = 'zeros'
        bias_init = tf.keras.initializers.Constant(-0.7) # 例: std=0.5から開始。tf.exp(-0.7)は約0.5 
        #bias_init = tf.keras.initializers.Constant(0.1) # 例: std=1.1から開始。tf.exp(0.1)は約1.1 
        ...
        # Clippingの値
        #self.min_log_std = -5.0 # tf.exp(-5.0) は約 0.0067
        #self.max_log_std = 2.0  # tf.exp(2.0)  は約 7.4
        #self.min_log_std = -6.0 # tf.exp(-6.0) は約 0.0024
        #self.max_log_std = 1.0  # tf.exp(1.0)  は約 2.7
        self.min_log_std = -5.0 # tf.exp(-5.0) は約 0.0067
        self.max_log_std = 2.0  # tf.exp(2.0)  は約 7.4
...
def create_actor_critic_models(input_shape, action_dim):
    ...
    actor_model(tf.zeros(shape=(1,)+input_shape,))
    critic_model(tf.zeros(shape=(1,)+input_shape,))
...
def get_action(actor_model, state, action_bound):
    """正規分布から行動をサンプリングする"""
    mu, log_std = actor_model(state.reshape((1, -1)))
    
    dist = tfp.distributions.Normal(loc=mu, scale=tf.math.exp(log_std))
    action_normalized = dist.sample()
    
    no_cliped_action = action_normalized[0]
    action = tf.clip_by_value(no_cliped_action, -action_bound, action_bound)
    
    #print("logstd:{:.2f} std:{:.2f} mu:{:.5f} action_norm:{:.2f} action_bound:{:.2f} no_clip:{:.2f} action:{:.2f}".format(
    #    log_std[0,0].numpy(),
    #    tf.math.exp(log_std)[0,0].numpy(),
    #    mu[0,0].numpy(),
    #    action_normalized[0,0].numpy(),
    #    action_bound,
    #    no_cliped_action[0].numpy(),
    #    action[0].numpy()
    #    )
    #)
...
def get_best_action(actor_model, state, action_bound):
    ...
    action = tf.clip_by_value(mu_normalized, -action_bound, action_bound)
...
def update(actor_model, critic_model, actor_optimizer, critic_optimizer, experiences, gamma, value_loss_weight, entropy_weight, standardize, action_bound):
    ...
    with tf.GradientTape() as actor_tape:
        # Actorのフォワードパス
        mu, log_std = actor_model(states, training=True)
        ...
        # 方策損失 (tf.stop_gradientでAdvantageへの勾配を止める)
    ...
    # ログ表示用の損失とエントロピーを返す
    return actor_loss.numpy(), critic_loss.numpy(), tf.reduce_mean(entropy).numpy(), tf.reduce_mean(std).numpy()

# === 学習ループ ===
# ★★★ 変更点 2: ログとプロットのために損失を個別に保持 ★★★
def train(env, actor_model, critic_model, action_bound):
    standardize = True
    total_timesteps = 130000#500000
    n_steps = 256#512
    gamma = 0.99
    actor_lr = 3e-4
    critic_lr = 1e-3
    ...
    for global_step in range(1, total_timesteps + 1):
        ...
        if len(experiences) >= n_steps:
            # 個別の損失を受け取る
            actor_loss, critic_loss, entropy, std = update(
        ...
        if (global_step % (n_steps * 5) == 0) or global_step == total_timesteps:
            ...
            last_entropy = np.mean(all_entropies[-20:]) if all_entropies else 0
            last_std = np.mean(all_stds[-20:]) if all_stds else 0
            # ログ表示を更新
            print(f'St:{global_step//1000}k|Ep:{episode_count}|AvgSt:{avg_episode_steps:.1f}|AvgRwd:{avg_reward:.1f}|'
                  f'ActorL:{last_actor_loss:.3f}|CriticL:{last_critic_loss:.3f}|Entr:{last_entropy:.3f}|Std:{last_std:.3f}')
...
if __name__ == '__main__':
    ...
    actor_model_file = 'pendulum-a2c-dlr4-actor3.weights.h5'
    critic_model_file = 'pendulum-a2c-dlr4-critic3.weights.h5'
    ...
    gif_path = 'pendulum-a2c-dlr4-nstep3.gif'
