...
def  train(model, optimizer, experiences):
...
for episode in range(iterations):
    ...
    while not (done or truncated):
        ...
        if len(experiences) == batch_size:
            step_loss,step_entropy = train(model,optimizer, experiences)
            loss += step_loss
            entropy += step_entropy
            experiences = []

    losses.append((loss/step)*500)
    entropies.append(500-(entropy/step)*500)
    rewards.append(total_reward)
    lap_loss += loss/step
    lap_entropy += entropy/step
    lap_reward += total_reward

    if(episode % snapshot == snapshot-1 and episode!=0):
        print('Episode {}/{}  reward:{:.1f} loss:{:.3f} entropy:{:.4f}'.format(episode+1,iterations,lap_reward/snapshot,lap_loss/snapshot,lap_entropy/snapshot))
        lap_loss = 0
        lap_entropy = 0
        lap_reward = 0
...
plt.plot(rewards)
plt.plot(entropy)
plt.legend(('reward','losses','entropy'))
plt.show()
